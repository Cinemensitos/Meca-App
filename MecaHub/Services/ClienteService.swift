        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard (200...299).contains(httpResponse.statusCode) else {
                ...
                if responseBody.contains("foreign key constraint") {
                    errorMsg = "No se puede eliminar este cliente porque tiene órdenes asociadas. Elimina primero sus órdenes."
                } else {
                    errorMsg = "Error al eliminar cliente (HTTP \(httpResponse.statusCode))"
                }
